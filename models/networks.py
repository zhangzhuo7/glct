...
class MultiAdaINResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        for i in range(n_blocks - n_blocks//2):       # add ResNet blocks
            model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        ...
        if mode == 'encode':
            ...
            if len(layers) > 0:
                ...
                if layers[-1] > layer_id:
                    self.assign_domain_variable(torch.zeros([len(input)]).to(input.device), self.decoder)
...
class AdaINResnetGenerator(nn.Module):
    ...
    def assign_domain_variable(self, domain_variable, model):
        ...
        assert len(domain_variable.size()) == 2
        if self.style_dim > 1:
            start = self.domain_embedding(torch.zeros([len(domain_variable)]).to(device).long())
            end = self.domain_embedding(torch.ones([len(domain_variable)]).to(device).long())
...
class GlobalImageDis(nn.Module):
    ...
    def _make_net(self, num_layers):
        dim = self.dim
        cnn_x = []
        cnn_x += [SN(nn.Conv2d(self.input_dim, dim, 4,2,1)), nn.LeakyReLU(0.2)]
        for i in range(num_layers):
            outdim = min(512, dim*2)
            cnn_x += [SN(nn.Conv2d(dim, outdim, 4, 2, 1)), nn.LeakyReLU(0.2)]
            dim = outdim
        cnn_x += [SN(nn.Conv2d(dim, 1, 4, 1, 0))]
...
class GlobalDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=5, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        if(no_antialias):
            ...
            sequence = [SN(nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=1, padding=padw)), nn.LeakyReLU(0.2, True), Downsample(ndf)]
        nf_mult = 1
        nf_mult_prev = 1
        ...
        sequence += [SN(nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=0))]  # output 1 channel prediction map
